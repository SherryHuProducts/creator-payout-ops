...
def test_no_historical_payment_is_ready_to_pay():
    ...
    assert result.variance == Decimal("-100.00")
...
def test_partial_payment_is_underpaid():
    ...
    assert result.outstanding_balance == Decimal("60.00")
    assert result.variance == Decimal("-60.00")
...
def test_excess_payment_is_overpaid():
    ...
    assert result.outstanding_balance == Decimal("-25.00")
    assert result.variance == Decimal("25.00")
...
def test_decimal_precision_and_currency_quantization():
    ...
    assert result.outstanding_balance == Decimal("3.34")
    assert result.variance == Decimal("-3.34")
    assert result.status is ReconciliationStatus.UNDERPAID


def test_payment_for_another_creator_is_ignored():
    result = reconcile_creator_payout(
        summary(), [payment("100.00", creator_id="C2")]
    )
    assert result.status is ReconciliationStatus.READY_TO_PAY
    assert result.amount_paid == Decimal("0.00")


def test_only_pending_payment_remains_not_paid():
    result = reconcile_creator_payout(
        summary("700.00"), [payment("700.00", PaymentStatus.PENDING)]
    )
    assert result.status is ReconciliationStatus.READY_TO_PAY
    assert result.amount_paid == Decimal("0.00")
    assert result.pending_payment_count == 1
